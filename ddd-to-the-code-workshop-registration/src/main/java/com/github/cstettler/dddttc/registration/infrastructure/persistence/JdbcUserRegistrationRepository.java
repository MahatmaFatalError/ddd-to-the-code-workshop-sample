...
class JdbcUserRegistrationRepository extends JsonBasedPersistenceSupport implements UserRegistrationRepository {
    ...
    JdbcUserRegistrationRepository(NamedParameterJdbcTemplate jdbcTemplate) {
        super(jdbcTemplate);
    }
    ...
    public UserRegistration get(UserRegistrationId userRegistrationId) throws UserRegistrationNotExistingException {
        try {
            return load("id", userRegistrationId.value());
    ...
    public UserRegistration find(UserHandle userHandle) {
        try {
            return load("user_handle", userHandle.value());
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    }
    ...
    private UserRegistration load(String attributeName, Object attributeValue) {
        return jdbcTemplate().queryForObject(
                "SELECT data FROM user_registration WHERE " + attributeName + " = :value",
                singletonMap("value", attributeValue),
